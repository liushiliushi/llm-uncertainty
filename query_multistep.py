...
# for prompt strategy
parser.add_argument("--prompt_type", type=str, default="multistep")
...
assert args.prompt_type == "multistep", "prompt_type must be multistep"
...
prompt_description =f"Read the question, break down the problem into K steps, think step by step, give your confidence in each step, and then derive your final answer and your confidence in this answer. Note: The confidence indicates how likely you think your answer is true.\nUse the following format to answer:\n\nStep 1: [Your reasoning], Confidence: [ONLY the confidence value that this step is correct]%\n...\nStep K: [Your reasoning], Confidence: [ONLY the confidence value that this step is correct]%\nFinal Answer and Overall Confidence (0-100): [ONLY the {answer_type}; not a complete sentence], [Your confidence value]%"
...
for idx, question in enumerate(qa_data.keys()):
    ...
    if idx % 5 == 0:
        ...
        final_json = {'hyperparameters': params, 'elapsed_time': "Elapsed time: {:.2f} seconds".format(elapsed_time), 'sample_tested': len(final_result), 'error_count':len(error_dataset), 'sample_prompt':{'question': sample_question, 'prompt': sample_prompt}, 'final_result': final_result, 'error_dataset': error_dataset}
...
end_time = time.time()
elapsed_time = end_time - start_time
final_json = {'hyperparameters': params, 'elapsed_time': "Elapsed time: {:.2f} seconds".format(elapsed_time), 'sample_tested': len(final_result), 'error_count':len(error_dataset), 'sample_prompt':{'question': sample_question, 'hint':sample_hint_prompt, 'prompt': prompt}, 'final_result': final_result, 'error_dataset': error_dataset} 
